...
const ProjectDetailsSection = ({ project, setProject, isExpanded, setIsExpanded }) => {
    const [editing, setEditing] = useState({
        title: false, description: false, price: false, video: false, deliveryLink: false
    });
    ...
    const handleSaveField = async (field) => {
        try {
            await protectedApi.patch(`/seller/project/update/${project.id}`, {
                title: form.title,
    ...
    const handleThumbnailUpdate = async (e) => {
        ...
        try {
            await protectedApi.patch(`/seller/project/${project.id}/thumbnail`, formData, {
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-8">
            <div className="flex-1 space-y-6">
                    ...
                    {editing.title ? (
                        <div className="flex items-center gap-2 w-full">
                            <input value={form.title} onChange={(e) => handleFieldChange("title", e.target.value)}
                                className="text-4xl font-bold bg-transparent border-b border-gray-400 text-[var(--text-color)] w-full" autoFocus />
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm mb-4 mt-2">
                    ...
                    <div className="flex items-center text-green-600 font-medium">
                        <ShoppingBag className="h-4 w-4 mr-1" /> 1,234 purchases
                    </div>
                </div>
                    ...
                    {editing.description ? (
                        <div className="flex items-start gap-2 w-full">
                            <textarea value={form.description} onChange={(e) => handleFieldChange("description", e.target.value)}
                                className="w-full bg-transparent border-b border-gray-400 text-[var(--text-secondary)]" rows={3} autoFocus />
                            <Save className="h-5 w-5 text-green-600 cursor-pointer mt-1" onClick={() => handleSaveField("description")} />
                        </div>
                    ) : (
                        <>
                            <p className="text-[var(--text-secondary)] mb-2">{isExpanded ? project.description : `${project.description?.slice(0, 80)}...`}</p>
                ...
                {project.description?.length > 80 && (
                    <button className="text-[var(--button-primary)] font-medium hover:underline" onClick={() => setIsExpanded(!isExpanded)}>
                ...
                <div className="flex gap-2 items-center">
                    ...
                    {editing.video ? (
                        <div className="flex items-center gap-2 w-full">
                            <input type="text" value={form.previewVideoUrl} onChange={(e) => handleFieldChange("previewVideoUrl", e.target.value)}
                                className="text-sm font-mono bg-transparent border-b border-gray-400 w-full" autoFocus />
                            <Save className="h-5 w-5 text-green-600 cursor-pointer" onClick={() => handleSaveField("video")} />
                        </div>
                    ) : (
                        <>
                            <p className="text-sm font-mono text-[var(--text-secondary)] truncate max-w-xs">{project.previewVideoUrl}</p>
                ...
                <div className="flex gap-2 items-center mt-2">
                    ...
                    {editing.deliveryLink ? (
                        <div className="flex items-center gap-2 w-full">
                            <input type="text" value={form.deliveryLink} onChange={(e) => handleFieldChange("deliveryLink", e.target.value)}
                                className="text-sm font-mono bg-transparent border-b border-gray-400 w-full" autoFocus />
                            <Save className="h-5 w-5 text-green-600 cursor-pointer" onClick={handleSaveDeliveryLink} />
                        </div>
                    ) : (
                        <>
                            <p className="text-sm font-mono text-[var(--text-secondary)] truncate max-w-xs">{project.deliveryLink || "Not set"}</p>
            ...
            <div className="rounded-xl shadow-lg p-6 w-full lg:w-80 bg-[var(--hover-bg)] border border-[var(--border-color)]">
                <div className="mb-4 relative group">
                    ...
                    <input id="thumbnailUpload" type="file" className="hidden" accept="image/*" onChange={handleThumbnailUpdate} />
                </div>
                ...
                <div className="space-y-4">
                    <div className="space-y-1">
                        <div className="flex items-baseline justify-between gap-2">
                            {editing.price ? (
                                <div className="flex items-center gap-2 w-full">
                                    <input type="number" value={form.price} onChange={(e) => handleFieldChange("price", e.target.value)}
                                        className="text-3xl font-bold text-[var(--button-primary)] bg-transparent border-b border-gray-400 w-full" autoFocus />
