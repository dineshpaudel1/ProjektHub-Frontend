...
import { protectedApi } from "../../../services/axiosInstance"; import { toast } from "react-toastify";
...
const QuestionAnswerList = ({ questions, refreshQuestions }) => {
    ...
    const handleSubmit = async (questionId) => {
        if (!answerMap[questionId]?.trim()) return;

        setSubmitting((prev) => ({ ...prev, [questionId]: true }));

        try {
            await protectedApi.post("/seller/interactions/answer-question", {
                questionId,
        ...
        <div className="mt-12">
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {questions.map((q) => {
                    ...
                    return (
                        <li key={questionId} className="bg-[var(--hover-bg)] border border-[var(--border-color)] p-4 rounded-lg text-sm shadow-md space-y-2">
                            <div>
